...
**Phase 3 品質チェックポイント:**
- [x] JPA EntityとドメインEntityの変換が正しく動作すること
- [x] インテグレーションテストが実際のH2 DBで動作すること
- [x] インフラ層がドメイン層のインターフェースを実装していること（依存方向が内側）
- [x] すべてのテストがグリーンであること
...
**Phase 4 品質チェックポイント:**
- [x] UseCaseがドメイン層とリポジトリインターフェースのみに依存していること
- [x] UseCaseにビジネスロジック（残高計算等）が漏れていないこと
- [x] すべてのテストがグリーンであること
...
**Phase 5 品質チェックポイント:**
- [x] 入出金のトランザクション管理が適切であること（@Transactional）
- [x] 残高不足の検証がドメイン層（Account.withdraw）で行われていること
- [x] UseCaseにドメインロジックが漏れていないこと
- [x] すべてのテストがグリーンであること
...
**Phase 6 品質チェックポイント:**
- [x] 全APIエンドポイントが仕様書のパス・メソッド・レスポンス形式と一致すること
- [x] バリデーションが適切に機能すること（@Valid, Bean Validation）
- [x] エラーレスポンスが統一フォーマットであること
- [x] すべてのテストがグリーンであること
